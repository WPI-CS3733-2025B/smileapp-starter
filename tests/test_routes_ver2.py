...
def init_database():
    ...
    user2 = new_user(uname='cookie', uemail='cookie@wpi.edu',passwd='1234')
...
"""
    Assumes the /post/<post_id>/like route returns the update JSON.
...
def test_index_page(test_client,init_database):
    """
    GIVEN a Flask application configured for testing , after user logs in,
    WHEN the index page is aloaded and SortForm' form is submitted (POST)
    THEN check that response is valid and the stories are sorted correctly
    """
    #login
    do_login(test_client, path = '/user/login', username = 'snow', passwd = '1234')

    #first add some stories
    story1 = Post(title='A very happy post', body='This is a very happy post.',happiness_level=2,user_id=1, likes=10)
    story1.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='funny')).first() )
    story1.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='heartwarming')).first() )
    story2 = Post(title='A cool story', body='This is a cool story.',happiness_level=3,user_id=1, likes=2)
    story2.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='inspiring')).first() )
    story2.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='true-story')).first() )
    story2.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='friendship')).first() )
    story3 = Post(title='An average day', body='This is an average day story.',happiness_level=1,user_id=1, likes=5)
    db.session.add_all([story1, story2, story3])    
    db.session.commit()
    # -------------------------------------------------
    # sort stories according to timestamp (descending)
    response = test_client.get('/index')
    #checking the page content after redirect
    assert response.status_code == 200  
    assert b"This is a very happy post." in response.data
    html_doc = response.data.decode('utf-8')
    soup = BeautifulSoup(html_doc, 'html.parser')
    post_titles = [h5.get_text() for h5 in soup.find_all('div', class_='card-title')]
    print(post_titles)
    # The posts should be sorted in descending order of timestamp
    assert post_titles == [' A very happy post  ', ' A cool story  ', ' An average day  ']
    #finally logout
    do_logout(test_client, path = '/user/logout') 
    
def test_sort_stories(test_client,init_database):
    """
    GIVEN a Flask application configured for testing , after user logs in,
    WHEN the index page is aloaded and SortForm' form is submitted (POST)
    THEN check that response is valid and the stories are sorted correctly
    ...
    db.session.commit()
    # -------------------------------------------------
    # sort stories according to like count (descending)
    response = test_client.post('/index', 
                          data=dict(sortorder='3'),
                          follow_redirects = True)
    #checking the page content after redirect
    assert response.status_code == 200
    assert b"This is a very happy post." in response.data 
    ...
    print(soup)
    post_titles = [h5.get_text() for h5 in soup.find_all('div', class_='card-title')]
    post_likes = [div.get_text() for div in soup.find_all('div', class_='likecount')]
    # The posts should be sorted in descending order of likes
    assert post_titles == [' A very happy post  ', ' An average day  ', ' A cool story  ']
    assert post_likes == [' 10 ',' 5 ', ' 2 '] 
    #-------------------------------------------------
    # sort stories according to happiness level (descending)
    response = test_client.post('/index',   
                            data=dict(sortorder='4'),
                            follow_redirects = True)
    #checking the page content after redirect   
    assert response.status_code == 200
    assert b"This is a very happy post." in response.data   
    ...
    soup = BeautifulSoup(html_doc, 'html.parser')
    post_titles = [h5.get_text() for h5 in soup.find_all('div', class_='card-title')]
    post_happiness = [div.get_text() for div in soup.find_all('div', class_='likecount')]
    # The posts should be sorted in descending order of happiness level 
    assert post_titles == [' An average day  ', ' A very happy post  ', ' A cool story  ']
    #-------------------------------------------------  
    # sort stories according to title (ascending)
    response = test_client.post('/index',   
                            data=dict(sortorder='2'),
                            follow_redirects = True)
    #checking the page content after redirect
    assert response.status_code == 200  
    assert b"This is a very happy post." in response.data
    ...
    soup = BeautifulSoup(html_doc, 'html.parser')
    post_titles = [h5.get_text() for h5 in soup.find_all('div', class_='card-title')]
    # The posts should be sorted in ascending order of title
    assert post_titles == [' A cool story  ', ' A very happy post  ', ' An average day  ']
    #-------------------------------------------------
    
    # sort stories according to timestamp (descending)
    response = test_client.post('/index',   
                            data=dict(sortorder='1'),
                            follow_redirects = True)
    #checking the page content after redirect
    assert response.status_code == 200  
    assert b"This is a very happy post." in response.data
...
def test_user_posts_only(test_client,init_database):
    """
    GIVEN a Flask application configured for testing , after user logs in,
    WHEN the index page is aloaded and SortForm' form is submitted (POST)
    THEN check that response is valid and the stories are sorted correctly
    ...
    #first add some stories
    story1 = Post(title='A very happy post', body='This is a very happy post.',happiness_level=2,user_id=2, likes=10)
    story1.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='funny')).first() )
    story1.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='heartwarming')).first() )
    story2 = Post(title='A cool story', body='This is a cool story.',happiness_level=3,user_id=1, likes=2)
    story2.tags.add( db.session.scalars(sqla.select(Tag).where(Tag.name =='inspiring')).first() )
    ...
    # sort stories according to like count (descending)
    response = test_client.post('/index', 
                          data=dict(sortorder='3', userpostsonly=True),
                          follow_redirects = True)
    #checking the page content after redirect
    assert response.status_code == 200
    assert b"This is a very happy post." not in response.data 
    assert b"This is a cool story."  in response.data 
    ...
    post_likes = [div.get_text() for div in soup.find_all('div', class_='likecount')]
    # The posts should be sorted in descending order of likes
    assert post_titles == [ ' An average day  ', ' A cool story  ']
    assert post_likes == [' 5 ', ' 2 '] 
    #finally logout
    do_logout(test_client, path = '/user/logout') 

def test_delete_story(test_client,init_database):
    """
    GIVEN a Flask application configured for testing , after user logs in,
    WHEN the index page is aloaded and SortForm' form is submitted (POST)
    THEN check that response is valid and the stories are sorted correctly
    ...
    # -------------------------------------------------
    # sort stories according to like count (descending)
    response = test_client.post('/post/{}/delete'.format(story2.id), 
                          data=dict(),
                          follow_redirects = True)
    #checking the page content after redirect
    assert response.status_code == 200
    assert b"This is a very happy post." in response.data 
    assert b"This is a cool story." not in response.data 

    #checking whether the database is updated correctly with the post request 
    allposts = db.session.scalars(sqla.select(Post)).all()
    assert len(allposts) == 2

    thepost = db.session.scalars(sqla.select(Post).where(Post.title =='This is a cool story.')).first()
    assert thepost is None  #the post should be deleted

    #finally logout
    do_logout(test_client, path = '/user/logout') 

if __name__ == '__main__':
    pytest.main(verbose=True)